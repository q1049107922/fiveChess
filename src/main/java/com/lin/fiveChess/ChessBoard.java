package com.lin.fiveChess;

/**
 * Created by lin on 17-8-20.
 */
import com.lin.common.util;
import com.lin.fiveChess.learning.ChessGame;
import com.lin.fiveChess.learning.ChessStep;
import com.lin.fiveChess.learning.fiveChessAIPlayer;
...
public class ChessBoard extends JPanel implements MouseListener {
   ...
   public static Point[] chessList=new Point[(ROWS+1)*(COLS+1)];//初始每个数组元素为null
   public boolean isBlack=true;//默认开始是黑棋先
   public boolean gameOver=false;//游戏是否结束
   public int chessCount;//当前棋盘棋子的个数
...
/*        ResultSet rs = null;
        try {
            rs = util.getConn().createStatement().executeQuery("select * from ChessGame where isWin=1 order by createTime DESC limit 1");

            ChessGame game = new ChessGame();
            while (rs.next()){
                game= ChessGame.fill(rs);
            }
            util.getConn().createStatement().execute("INSERT INTO ChessGame (isWin,color,createTime) value (0,1,'"+util.getDateTimeNow()+"')");

            fiveChessAIPlayer aiPlayer1 = new  fiveChessAIPlayer(game.id+1,this);
            fiveChessAIPlayer aiPlayer2 = new  fiveChessAIPlayer(game.id+2,this);
            int no = 1;
            while(!isWin()){
                aiPlayer1.autoPlay(no);
                Thread.sleep(1000);
                aiPlayer2.autoPlay(no);
                no=no+1;
                Thread.sleep(1000);
            }
    ...
    public void printPoint(ChessStep step) throws SQLException {
        ...
        if(isWin()){
            util.getConn().createStatement().execute("update ChessGame set isWin=1 where id="+ step.gameId);
            String msg=String.format("恭喜，%s赢了！", colorName);
            JOptionPane.showMessageDialog(this, msg);
            gameOver=true;
        }
    ...
    public static boolean findChess(int x,int y){
